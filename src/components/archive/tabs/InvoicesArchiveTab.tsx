...
import { Download, Search, DollarSign, Printer, Trash2, AlertTriangle } from 'lucide-react';
import { exportInvoicePdf, exportInvoicesCollectionPdf } from '@/lib/archivePdfExport';
...
export default function InvoicesArchiveTab({ data, onDownload, onRefresh }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return localData.filter((inv: any) => {
  ...
  const handleDelete = useCallback(async (item: Record<string, unknown>) => {
    ...
    if (!id) { setFeedback({ ok: false, msg: 'لا يمكن حذف فاتورة بدون معرف' }); return; }
    setDeletingId(id);
    const res = await deleteArchiveItem('invoices', id);
    setDeletingId(null);
    setDeleteConfirm(null);
    if (res.success) {
      setLocalData(prev => prev.filter((i: any) => i.id !== id));
      setFeedback({ ok: true, msg: '✅ تم حذف الفاتورة بنجاح' });
    } else {
  ...
  return (
    <div className="p-5" dir="rtl">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" dir="rtl">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-slate-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-black text-slate-900">تأكيد حذف الفاتورة</p>
                <p className="text-xs text-slate-500 mt-0.5">هذه العملية لا يمكن التراجع عنها</p>
              </div>
            </div>
            <p className="text-sm text-slate-700 mb-4">
              هل أنت متأكد من حذف الفاتورة رقم <strong>{String((deleteConfirm as any).invoiceNumber || (deleteConfirm as any).id || '—')}</strong> الخاصة بالطالب <strong>{String((deleteConfirm as any).studentName || '—')}</strong>؟
            </p>
      ...
      <div className="mb-4 flex flex-wrap gap-3 items-center">
        ...
        <button onClick={() => exportInvoicesCollectionPdf(filtered)}
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200">
        <table className="min-w-full text-xs">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              {['رقم الفاتورة', 'الطالب', 'ولي الأمر', 'المبلغ', 'الوصف', 'الحالة', 'تاريخ الاستحقاق', 'تاريخ الإنشاء', 'إجراءات'].map(h => (
          ...
          <tbody className="divide-y divide-slate-100">
              ...
              <tr key={inv.id ?? i} className="hover:bg-slate-50 transition">
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => exportInvoicePdf(inv)} title="تصدير PDF"
                    ...
                    <button onClick={() => setDeleteConfirm(inv)} title="حذف" disabled={deletingId === String(inv.id)}
            ...
            {pageData.length === 0 && <tr><td colSpan={9} className="py-16 text-center text-slate-400 text-sm">لا توجد فواتير</td></tr>}
          </tbody>
        </table>
      </div>
