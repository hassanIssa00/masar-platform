      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 p-3">
          {(!collapsed || isMobileShow) && (
            <span className="text-xs font-black uppercase tracking-wider text-slate-400">تشغيل المنصة</span>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-2">
            ...
            {adminLinks.map(({ name, path, icon: Icon }) => {
              ...
              return (
                ...
                >
                  ...
                  {(!collapsed || isMobileShow) && (
                    <ChevronLeft size={14} className={isActive ? 'text-teal-600' : 'text-slate-300 group-hover:text-teal-500 transition'} />
                  )}
                </Link>
              );
            })}
          ...
          <div className="mt-5 border-t border-slate-100 pt-4">
            {(!collapsed || isMobileShow) && (
              <p className="text-xs font-black uppercase tracking-wider text-slate-400 mb-3 px-2">المسارات العلاجية</p>
            )}
            <div className="grid gap-1">
              {curriculumPrograms.map((program) => {
                const programPath = `/programs/${program.slug}`;
                const isActive = pathname === programPath;
                return (
                  <Link
                    key={program.slug}
                    href={programPath}
                  ...
                  >
                    ...
                    {(!collapsed || isMobileShow) && <span className="mt-1.5 block h-1 rounded-full" style={{ backgroundColor: program.color }} />}
                  </Link>
                );
              })}
            </div>
        ...
        <div className="border-t border-slate-200 p-2">
          <button
            ...
            className={`
              flex items-center justify-center rounded-xl border border-rose-100 bg-rose-50 text-xs font-black text-rose-700 hover:bg-rose-100 transition
              ${collapsed && !isMobileShow ? 'h-10 w-full' : 'w-full gap-2 px-4 py-2.5'}
            `}
          >
            <LogOut size={16} />
