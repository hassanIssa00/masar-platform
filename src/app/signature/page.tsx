...
export default function SignaturePage() {
  ...
  useEffect(() => { setHijri(getHijriParts()); }, []);
  ...
  const downloadStamp = () => {
    ...
    img.onload = () => {
      ...
      a.download = `ختم-د-إسماعيل-${hijri.day}-${hijri.month}-${hijri.year}هـ.png`;
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="hidden sm:inline-flex items-center gap-1.5 text-xs font-black text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1 rounded-full">
              <Stamp size={13} /> بوابة الختم الإلكتروني الرسمي
            </span>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={downloadStamp}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-700 text-white font-black text-xs transition active:scale-95 shadow"
            >
              <Download size={14} /> تحميل الختم PNG
            </button>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10 sm:py-14">
        ...
        <div className="text-center mb-10 space-y-2">
          ...
          <p className="text-sm font-bold text-gray-500">
            التاريخ الهجري يتحدث تلقائياً كل يوم داخل الختم — حمّله وضعه على أي تقرير أو مستند
          </p>
          {hijri.fullLong && (
            <p className="text-base font-black text-teal-700">
              📅 اليوم: {hijri.fullLong} هـ
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10 items-start justify-center">
          ...
          <div className="flex flex-col items-center gap-5">
            ...
            <div className="bg-white rounded-3xl border border-gray-200 shadow-xl p-8 flex items-center justify-center">
              ...
              >
                ...
                {/* Ring dots between outer and inner circles */}
                {ringDots(CX, CY, (RO + RI) / 2, 72, INK)}

                {/* Inner circle — white fill so it covers dots behind it */}
                ...
                {/* ── Handwritten signature (ENLARGED & PERFECTLY CENTERED) ── */}
                {sigB64 && (
                  <image
                    href={sigB64}
                    x={CX - 170}
            ...
            <button
              onClick={downloadStamp}
              className="flex items-center gap-2 px-8 py-3.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-black text-sm transition shadow-lg active:scale-95 w-full max-w-xs justify-center"
            >
              <Download size={18} /> تحميل الختم (PNG شفاف)
            </button>
          ...
          <div className="flex-1 space-y-5 max-w-sm">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
              <h3 className="font-black text-gray-900 text-base flex items-center gap-2">
                <CheckCircle2 size={18} className="text-teal-600" /> تفاصيل الختم
              </h3>
              <div className="space-y-3 text-sm divide-y divide-gray-100">
                <div className="flex justify-between pb-2">
                  <span className="text-gray-500 font-bold">صاحب الختم</span>
                  <span className="font-black text-gray-900">د. إسماعيل عيسى</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-gray-500 font-bold">التاريخ اليوم هجري</span>
                  <span className="font-black text-teal-700">{hijri.fullLong || '…'} هـ</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-gray-500 font-bold">اليوم</span>
                  <span className="font-black text-slate-800">{hijri.day || '…'}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-gray-500 font-bold">الشهر</span>
                  <span className="font-black text-slate-800">{hijri.month || '…'}</span>
                </div>
                <div className="flex justify-between pt-2">
                  <span className="text-gray-500 font-bold">السنة الهجرية</span>
                  <span className="font-black text-slate-800">{hijri.year || '…'} هـ</span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-emerald-700 mt-1 leading-relaxed">
                  التاريخ الهجري داخل الختم يتغير تلقائياً كل يوم — كل ما تفتح الصفحة وتحمّل الختم يكون بتاريخ اليوم الفعلي
                </p>
              </div>
            </div>
