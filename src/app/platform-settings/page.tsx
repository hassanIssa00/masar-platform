                          ...
                          {filteredAccounts.map((acc) => (
                            <tr key={acc.id} className="hover:bg-slate-50 transition">
                              ...
                              <td className="px-4 py-3">
                                <button
                                  onClick={async () => {
                                    if (!confirm(`هل تريد حذف حساب "${acc.name}" نهائياً من قاعدة البيانات والسحابة؟\n\n⚠️ سيتم أرشفة بصمة الوجه وحفظها في الأرشيف قبل الحذف.`)) return;
                                    try {
                                      const { archiveAndPurgeAccount } = await import('@/lib/accountArchiver');
                                      const result = await archiveAndPurgeAccount(acc.id, 'حذف يدوي من لوحة التحكم');
                                      if (result.success) {
                                      ...
                                      console.error('Archive/purge failed:', err);
                                      // Fallback: raw delete
