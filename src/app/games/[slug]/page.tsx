  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-950 text-white" dir="rtl">
      <header className="flex items-center justify-between gap-3 border-b border-white/10 bg-slate-900 px-4 py-3">
        <Link href="/kids" className="inline-flex items-center gap-2 rounded-lg bg-white/10 px-4 py-2 text-sm font-black text-white hover:bg-white/15">
          <ArrowRight size={17} />
          الألعاب
        </Link>
...
function GameShell({
  ...
  return (
    <section className="flex h-[calc(100svh-65px)] flex-col">
      <div className="flex items-center justify-between gap-3 border-b border-white/10 bg-slate-900/70 px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onReset} className="grid h-10 w-10 place-items-center rounded-lg bg-white/10 text-white hover:bg-white/15" title="إعادة">
            <RotateCcw size={18} />
          </button>
        </div>
      </div>
...
function TargetsGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="اضغط الأهداف المتحركة بسرعة" onReset={reset}>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,#334155,#020617)]" />
      {targets.map((target) => (
        <button
          key={target.id}
          onClick={() => {
          ...
          className="absolute rounded-full border-4 border-white shadow-2xl transition hover:scale-110"
      ...
      <button onClick={() => setRunning((value) => !value)} className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-lg bg-white px-6 py-3 text-sm font-black text-slate-950">
...
function CollectorGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="حرّك الشريط واجمع القطع الساقطة" onReset={reset}>
      <div className="absolute inset-0 bg-[linear-gradient(180deg,#111827,#020617)]" />
      {items.map((item) => (
      ...
      <div className="absolute bottom-12 h-8 w-28 -translate-x-1/2 rounded-lg bg-white shadow-2xl" style={{ left: `${player}%` }} />
      <div className="absolute inset-x-4 bottom-3 flex justify-between">
        <button onClick={() => setPlayer((value) => Math.max(10, value - 10))} className="h-14 w-28 rounded-lg bg-white/15 text-2xl font-black text-white">يمين</button>
        <button onClick={() => setPlayer((value) => Math.min(90, value + 10))} className="h-14 w-28 rounded-lg bg-white/15 text-2xl font-black text-white">يسار</button>
      </div>
    </GameShell>
  );
}
...
function RacerGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="غيّر المسار وتجنب العوائق" onReset={reset}>
      ...
      {crashed && <div className="absolute inset-0 grid place-items-center bg-slate-950/70 text-center"><button onClick={reset} className="rounded-lg bg-white px-8 py-4 text-xl font-black text-slate-950">إعادة اللعب</button></div>}
      <div className="absolute inset-x-4 bottom-3 flex justify-between">
      ...
      <div className="grid h-full place-items-center p-4">
        <div className="grid w-full max-w-xl grid-cols-4 gap-3">
          {cards.map((card, index) => {
            ...
            return (
              <button key={`${card}-${index}`} onClick={() => click(index)} className="aspect-square rounded-lg border border-white/20 text-xl font-black shadow-lg" style={{ backgroundColor: visible ? color : '#ffffff', color: visible ? 'white' : '#0f172a' }}>
                {visible ? card : ''}
              </button>
            );
          })}
...
function PaintGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="ارسم بالقلم داخل المساحة البيضاء" onReset={reset}>
      <div className="grid h-full place-items-center p-4">
        <canvas ref={canvasRef} width={900} height={520} onPointerDown={() => setDrawing(true)} onPointerUp={() => setDrawing(false)} onPointerMove={draw} className="h-full max-h-[520px] w-full max-w-5xl touch-none rounded-lg bg-white shadow-2xl" />
      </div>
    </GameShell>
  );
}
...
function SnakeGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="حرّك المربع واجمع النقطة" onReset={() => { setHead({ x: 5, y: 5 }); setScore(0); }}>
      <div className="grid h-full place-items-center p-4">
        <div className="grid aspect-square w-full max-w-[520px] grid-cols-12 gap-1 rounded-lg bg-white/10 p-2">
...
function PianoGame({ color }: { color: string }) {
  ...
  return (
    <GameShell score={score} title="اضغط المفاتيح واستمع للنغمات" onReset={() => setScore(0)}>
      <div className="grid h-full place-items-center p-4">
        <div className="flex h-72 w-full max-w-3xl items-end gap-2 rounded-lg bg-white p-3 shadow-2xl">
